 ...
 * 
 * 배열 인덱스:     [0]         [1]             [2]
 * 메모리:      [101~105번지][105~108번지][109번지~112번지]
 * 실제 값:         7           8               9
 * 
 *  - *(arrNum + 0) == arrNum[0]  // arrNum은 101번지, 101+0 = 0, 앞의 *(간접참조연산자)에 의해서 101번지의 값 == arrNum[0]의 값
 *  - *(arrNum + 1) == arrNum[1]  // 101+1(4바이트)= 105, *에 의해서 105번지의 값 == arrNum[1]
 *  - *(arrNum + 2) == arrNum[2]
 * 
 * 일반변수 int num = 4;
 ...
 * 
 * pa = &num;
 * pa = arrNum;  // 배열은 변수명자체가 주소이기 때문에 &을 사용하지않음!
 * 
 * 배열요소(아이템) 접근방법
 *  int *pa:  // 101번지
 *  1. 정수연산 pa+1    100+1(int:4 Byte) → 105번지(2번째 요소)
